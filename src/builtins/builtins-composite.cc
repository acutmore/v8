...
BUILTIN(CompositeConstructor) {
  ...
  if (target.is_identical_to(root_composite)) {
    ...
    // Store metadata directly as a property on the constructor function.
    // Use a private symbol to hide it from JavaScript.
    ...
    meta_data->set(2, Smi::FromInt(static_cast<int>(shape_seed)));  // Shape seed    // Store metadata using a private symbol as the key.
    DirectHandle<Symbol> meta_key = factory->NewPrivateSymbol();
    MaybeDirectHandle<Object> set_result = JSObject::SetOwnPropertyIgnoreAttributes(
        ctor, meta_key, meta_data,
        static_cast<PropertyAttributes>(DONT_ENUM | DONT_DELETE | READ_ONLY));
    if (set_result.is_null()) {
      return ReadOnlyRoots(isolate).exception();
    }

    // Store the symbol on the constructor for fast retrieval.
    // We'll use a well-known index in the FixedArray for this.
    DirectHandle<FixedArray> meta_wrapper = factory->NewFixedArray(2);
    meta_wrapper->set(0, *meta_key);   // The private symbol key
    meta_wrapper->set(1, *meta_data);  // The actual metadata

    // Store this in a property with a predictable name for fast access.
  ...
  if (!target.is_identical_to(root_composite)) {
    ...
    DirectHandle<FixedArray> descriptor_indices(descriptor_indices_tagged, isolate);    // Allocate with initial map, then migrate - this ensures proper initialization.
  ...
  for (InternalIndex i : a_map->IterateOwnDescriptors()) {
    ...
    if (details.location() == PropertyLocation::kField &&
      ...
      if (IsJSComposite(av) && IsJSComposite(bv)) {
        DirectHandle<JSComposite> a_comp(Cast<JSComposite>(av), isolate);
        DirectHandle<JSComposite> b_comp(Cast<JSComposite>(bv), isolate);
        if (!CompareComposites(isolate, a_comp, b_comp)) {
          return false;
        }
      } else {
